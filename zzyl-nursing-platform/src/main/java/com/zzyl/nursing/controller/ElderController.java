...
@GetMapping("/list")
@ApiOperation("查询老人列表")
    public TableDataInfo<List<Elder>> list(@ApiParam(value = "老人查询条件") Elder elder)
    {
        startPage();
    ...
    @PostMapping("/export")
    @ApiOperation("导出老人列表")
    public void export(HttpServletResponse response, @ApiParam(value = "老人查询条件") Elder elder)
    {
        List<Elder> list = elderService.selectElderList(elder);
    ...
    @DeleteMapping("/{ids}")
    @ApiOperation("删除老人")
    public AjaxResult remove(@ApiParam(value = "老人ID数组", required = true) @PathVariable Long[] ids)
    {
