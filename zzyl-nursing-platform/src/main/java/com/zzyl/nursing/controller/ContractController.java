...
@GetMapping("/list")
@ApiOperation("查询合同列表")
    public TableDataInfo<List<Contract>> list(@ApiParam(value = "合同查询条件") Contract contract)
    {
        startPage();
    ...
    @PostMapping("/export")
    @ApiOperation("导出合同列表")
    public void export(HttpServletResponse response, @ApiParam(value = "合同查询条件") Contract contract)
    {
        List<Contract> list = contractService.selectContractList(contract);
    ...
    @DeleteMapping("/{ids}")
    @ApiOperation("删除合同")
    public AjaxResult remove(@ApiParam(value = "合同ID数组", required = true) @PathVariable Long[] ids)
    {
